#!/usr/bin/env bash
# blockperf-install-amaru.sh - Install OpenBlockPerf beside an Amaru (Rust) node
#
# Dedicated installer for Amaru environments (no CNTools / TraceOptions).
# Phase A: systemd + journald (typical .deb / .rpm unit). Nix / Homebrew /
# Docker: pass --node-unit-name and/or --tracer-log-file explicitly.
#
# This script:
...
#   4. Writes a config file with node_kind=amaru (journal default)
#   5. Writes a systemd service unit PartOf= the Amaru unit
#
# Usage:
#   sudo ./blockperf-install-amaru.sh
#   sudo ./blockperf-install-amaru.sh --node-unit-name amaru.service --network mainnet
#   sudo ./blockperf-install-amaru.sh --yes --api-key-mode relay
#   sudo ./blockperf-install-amaru.sh --update
#   sudo ./blockperf-install-amaru.sh --remove
#
# For the Haskell cardano-node installer, use blockperf-install.sh instead.
#
...
#   NODE_UNIT_NAME    systemd unit for Amaru (e.g. amaru.service).
#                     Default: auto-discover; use --node-unit-name to set explicitly.
#                     Also used as config node_unit_name in journald mode.
#   TRACER_LOG_FILE   Optional absolute path to an Amaru JSON logfile.
#                     Default for Amaru is journald (unit StandardOutput=journal).
#   CONFIG_NODE_UNIT_NAME
#                     Optional override for config.json node_unit_name.
#                     Journald: usually the Amaru unit. Logfile: often "" (accept all).
#   LOCAL_PORT        Amaru listen port written to config (default 3000;
#                     stock amaru --listen-address / AMARU_LISTEN_ADDRESS).
#   NODE_CONFIG_PATH  Unused for Amaru (kept empty). Accepted for CLI compatibility.
...
INSTALLER_REMOTE_URL="https://raw.githubusercontent.com/cardano-foundation/openblockperf/main/blockperf-install-amaru.sh"
...
print_intro_and_confirm() {
    echo
    echo -e "${BOLD}OpenBlockPerf Amaru installer overview${NC}"
    echo "  Installer Version: ${INSTALLER_VERSION}"
    echo "  Target: Amaru Rust node (journald / systemd). Not for Haskell CNTools."
    echo "  1) Check/install prerequisites (Debian/Ubuntu and RHEL-family)"
    echo "  2) Configure service user, node name, Amaru unit, listen port, log source"
...
Options:
  ...
  --node-unit-name <unit>
                 systemd unit for Amaru (e.g. amaru.service). Skips auto-discovery.
                 Same as NODE_UNIT_NAME=...
  --local-port <port>
                 Amaru listen port for config local_port (default 3000).
                 Probed from AMARU_LISTEN_ADDRESS in /etc/default/amaru when unset.
                 Same as LOCAL_PORT=...
  --node-config <path>
                 Ignored for Amaru (kept for CLI compatibility).
  --tracer-log-file <path>
                 Optional Amaru JSON logfile. Default is journald (recommended for
                 stock amaru.service with StandardOutput=journal).
                 Same as TRACER_LOG_FILE=...
  --network mainnet|preprod|preview
                 Cardano network. Probed from AMARU_NETWORK in /etc/default/amaru
                 when unset. Same as NETWORK=...
  --node-name <name>
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --node-unit-name)
                ...
                ;;
            --node-config)
                [[ $# -ge 2 ]] || die "--node-config requires a path"
                CLI_NODE_CONFIG="$2"
                shift 2
                ;;
            --local-port)
                [[ $# -ge 2 ]] || die "--local-port requires a port number"
                LOCAL_PORT="$2"
                shift 2
                ;;
            --tracer-log-file)
...
score_amaru_unit_candidate() {
    local unit="$1"
    local name_l desc ex hay score reason active_state
    name_l="${unit,,}"
    ...
    case "${hay}" in
        *exporter*|*monitor*|*metrics*|*prometheus*|*grafana*)
            score=$((score - 120))
            reason="monitor/exporter-like service"
            ;;
    esac

    if [[ "${ex,,}" == *"/usr/bin/amaru"* || "${ex,,}" == *" amaru run"* || "${ex,,}" == *"amaru"* ]]; then
        score=$((score + 120))
        reason+=", ExecStart has amaru"
    fi
    case "${name_l}" in
        amaru.service)
            score=$((score + 80))
...
resolve_amaru_unit() {
    ...
    if [[ -n "${NODE_UNIT_NAME}" ]]; then
        ...
        info "Amaru unit: ${NODE_UNIT_NAME} (explicit)"
        return 0
    fi
    ...
    done < <(collect_amaru_unit_candidates)
    local n=${#candidates[@]}

    if [[ "${n}" -eq 1 ]]; then
        NODE_UNIT_NAME="${candidates[0]}"
        validate_node_unit_or_die "${NODE_UNIT_NAME}"
        info "Amaru unit: ${NODE_UNIT_NAME} (auto-detected)"
        return 0
    fi
    ...
    if [[ "${n}" -gt 1 ]]; then
        ...
        if pick_best_amaru_unit_candidate "${candidates[@]}"; then
            auto_pick_ok=true
        fi
        ...
        if [[ "${ASSUME_YES}" == "true" ]]; then
            if [[ "${auto_pick_ok}" == true ]]; then
                ...
                validate_node_unit_or_die "${NODE_UNIT_NAME}"
                info "Amaru unit: ${NODE_UNIT_NAME} (auto-selected from ${n} candidates; score ${NODE_UNIT_CANDIDATE_SCORE})"
                return 0
            fi
            die "Multiple Amaru units found (${n}) and no confident auto-pick for --yes mode.
Candidates:
${NODE_UNIT_CANDIDATE_REPORT}
Set NODE_UNIT_NAME= or --node-unit-name to choose one."
        fi
        has_prompt_tty || die "Multiple Amaru units found. Set NODE_UNIT_NAME= or --node-unit-name."
        echo
        warn "Multiple systemd units matching amaru were found:"
        if [[ "${auto_pick_ok}" == true ]]; then
        ...
        if [[ "${auto_pick_ok}" == true ]]; then
            ...
            if [[ -z "${sel}" ]]; then
                NODE_UNIT_NAME="${NODE_UNIT_CANDIDATE_PICK}"
                validate_node_unit_or_die "${NODE_UNIT_NAME}"
                info "Amaru unit: ${NODE_UNIT_NAME}"
                return 0
            fi
        else
            prompt_read sel "Select 1-${n}, or type a full unit name (e.g. amaru.service): " || die "No unit selected."
        fi
    ...
    if [[ "${ASSUME_YES}" == "true" ]]; then
        die "No Amaru systemd unit found. Install the amaru package or set --node-unit-name amaru.service."
    fi
    has_prompt_tty || die "No Amaru systemd unit found. Set NODE_UNIT_NAME= or --node-unit-name."
    local typed=""
    prompt_read typed "Amaru systemd unit name [amaru.service]: " || true
    NODE_UNIT_NAME="$(normalize_node_unit_name "${typed:-amaru.service}")"
    validate_node_unit_or_die "${NODE_UNIT_NAME}"
    info "Amaru unit: ${NODE_UNIT_NAME}"
}

# Keep the Haskell-era function name so main() stays unchanged.
...
resolve_log_source() {
    ...
    local ans=""
    echo
    prompt_read ans "Read Amaru logs from journald (recommended for stock amaru.service)? [Y/n]: " || true
    case "${ans}" in
        n|N|no|NO)
            local p=""
            prompt_read p "Absolute path to Amaru JSON logfile: " || die "No logfile path given."
...
resolve_node_config_path() {
    # Amaru has no TraceOptions config.json. Keep empty unless operator forced a path.
    if [[ -n "${CLI_NODE_CONFIG}" ]]; then
        NODE_CONFIG_PATH="${CLI_NODE_CONFIG}"
        info "Node config path set explicitly (unused by Amaru parser): ${NODE_CONFIG_PATH}"
        return 0
    fi
    NODE_CONFIG_PATH=""
    info "Node config: (none; Amaru uses EnvironmentFile / amaru run)"
}

resolve_local_port() {
    # Prefer explicit LOCAL_PORT / --local-port. Else AMARU_LISTEN_ADDRESS
    # (amaru maps --listen-address → AMARU_LISTEN_ADDRESS, default 0.0.0.0:3000).
    local f line val
    for f in /etc/default/amaru /etc/sysconfig/amaru; do
        [[ -r "${f}" ]] || continue
        line="$(grep -E '^(AMARU_LISTEN_ADDRESS|LISTEN_ADDRESS|AMARU_LISTEN|LISTEN_ADDR)=' "${f}" 2>/dev/null | head -n1 || true)"
        if [[ -n "${line}" ]]; then
            val="${line#*=}"
            val="${val//\"/}"
            val="${val##*:}"
            if [[ "${val}" =~ ^[0-9]+$ ]]; then
                LOCAL_PORT="${val}"
                info "Local port: ${LOCAL_PORT} (from ${f})"
                return 0
            fi
        fi
    done
    ...
    if [[ "${ASSUME_YES}" == "true" ]] || ! has_prompt_tty; then
        LOCAL_PORT="${LOCAL_PORT:-3000}"
        info "Local port: ${LOCAL_PORT} (Amaru default)"
        return 0
    fi

    local ans=""
    prompt_read ans "Amaru listen port [${LOCAL_PORT:-3000}]: " || true
    LOCAL_PORT="${ans:-${LOCAL_PORT:-3000}}"
    [[ "${LOCAL_PORT}" =~ ^[0-9]+$ ]] || die "Invalid LOCAL_PORT '${LOCAL_PORT}'"
    info "Local port: ${LOCAL_PORT}"
}

# Step 5 — Cardano network
derive_network_from_amaru_env() {
    local f line val
    for f in /etc/default/amaru /etc/sysconfig/amaru; do
        [[ -r "${f}" ]] || continue
        # amaru maps --network → AMARU_NETWORK
        line="$(grep -E '^(AMARU_NETWORK|NETWORK|CARDANO_NETWORK)=' "${f}" 2>/dev/null | head -n1 || true)"
        if [[ -n "${line}" ]]; then
            val="${line#*=}"
            val="${val//\"/}"
            val="${val,,}"
...
resolve_network() {
    ...
    local derived=""
    derived="$(derive_network_from_amaru_env 2>/dev/null || true)"
    if [[ -n "${derived}" ]]; then
        NETWORK="${derived}"
        info "Network: ${NETWORK} (from Amaru EnvironmentFile)"
        check_network_value
        return 0
    fi
...
write_config_file() {
    ...
    cat > "${CONFIG_FILE}" <<EOF
{
  "_comment": "OpenBlockPerf client (Amaru). See docs/blockperf-install-amaru.md",
...
write_service_file() {
    ...
    # the node unit so peer-event state resets when amaru is restarted.
    cat > "${SERVICE_FILE}" <<EOF
[Unit]
Description=OpenBlockPerf Client (Amaru)
...
main() {
    ...
    if [[ "${MODE}" == "remove" ]]; then
        echo
        echo -e "${BOLD}OpenBlockPerf Amaru Installer (${MODE})${NC}"
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    installer_step_banner 2 5 "Configure service user, node name, Amaru unit, listen port, log source..."
    check_python
    resolve_service_identity
    resolve_node_name
    resolve_cardano_node_unit
    ...
    printf "  %-14s %s\n" "Node config:"  "${NODE_CONFIG_PATH:-(none)}"
    printf "  %-14s %s\n" "Network:"      "${NETWORK}"
    printf "  %-14s %s\n" "Config file:"  "${CONFIG_FILE}"
